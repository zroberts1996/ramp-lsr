...
export class DrawToolbar {
    ...
    constructor(mapApi: any, config: any) {
        ...
        /*this.panel = this._mapApi.panels.create('customForm');
        this.panel.element.css(this._panelOptionsForm);
        this.panel.body = this.makeTable(this.GlobalSearchOutput)
    ...
    QueryLayer(ext) {
        //selects polygons within the extent of drawing tools polygon
          ...
          //console.log(Object.values(updatedInput))
          //this.panel.open();
          console.log(updatedInput[0])
          //var objUpdatedInput = Object.keys(updatedInput).map(i => updatedInput[i])
          //console.log(objUpdatedInput)
    ...
    addToMap(evt: any) {
        switch ((<any>evt).geometry.type) {
            ...
            case 'polygon':
                ...
                var Output = this.QueryLayer(evt.geometry)
                break;
            case 'extent':
                // trigger observable
